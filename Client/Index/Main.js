...
const URLS = {
  ...
  URL_CREATE_CART : `https://localhost:7075/api/Cart/user/createCart/` // <- id pending
}
...
document.getElementById('authForm').addEventListener('submit', async function(event) {
  // Solo proceder si el correo es válido (ya lo verificaste en el listener anterior)
    ...
    if (isLogin) {
        ...
        await login(URLS.URL_LOGIN, URLS.URL_CREATE_CART, dataLogin);
...
async function login(url, urlCart, body){
    try {
        ...
        swal({
            ...
            if (!user.isActive) {
                ...
                const fullUrl = `${urlCart}${user.id}`;
                await apiCall(fullUrl, "POST");
                window.location = "../subpages/home/Home.html";
            } else {
